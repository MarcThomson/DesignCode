...
if Perfusion
    ...
    VCD = 1-0.5259/(1+353.3*exp(-0.9381*t)); % emperical function fit to data
end

%% Call other parameters
parameterDefinitions;
load('reversibleLogicals.mat');
load('internalLogicals.mat');
...
numRxns = length(stoichMatrix(1,:));        %define the number of reactions
numComponents = length(stoichMatrix(:,1));  %define the number of components

% define a new vector including all components for MFB
I_external = find(internal == 0); % indices of external components
C_new = zeros(numComponents,1);
C_new(I_external) = C;

%% Optimization Initialization
% make sure the system is satisdied to a very low tolerance
tol = 1e-10;
