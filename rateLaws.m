...
V_AKGDH = V_maxAKGDH * c_AKG/(K_mAKG+c_AKG) * c_NAD/c_NADH/(K_mNAD_NADH+c_NAD/c_NADH); 
    ...
    %Checked 1/28/18 Ed
%14
V_SUC = V_maxSUC * c_SUC/(K_mSUC+c_SUC) * (c_NAD/c_NADH)/(K_mNAD_NADH+c_NAD/c_NADH)...
        * (c_ADP/c_ATP)/(K_mADP_ATP+c_ADP/c_ATP);
    %mmol/1E6 cells/hr
    %Checked 1/28/18 Ed
...
V_PC = V_maxPC * c_PYR/(K_mPYR+c_PYR) * (c_ADP/c_ATP)/(K_mADP_ATP+c_ADP/c_ATP);
    ...
    %Checked 1/28/18 Ed
%17
V_ME = V_maxME * c_MAL/(K_mMAL+c_MAL) * c_NAD/c_NADH/(K_NAD_NADH+c_NAD/c_NADH);
    %mmol/1E6 cells/hr
    %Checked 1/28/18 Ed
%18
...
V_PPRibP = V_maxPPRibP * c_R5P/(K_mR5P+c_R5P) * c_EGLN/(K_mEGLN+c_EGLN) ...
    ...
    %mmol/1E6 cells/hr
    %Checked 1/28/18 Ed
%33 BIG AUTHOR TYPO IN THAT TYR IS DOUBLE COUNTED FOR
V_growth = V_maxgrowth * c_G6P/(K_mG6P_growth+c_G6P) * c_R5P/(K_mR5P_growth+c_R5P) ...
           ...
           * c_THR/(K_mTHR_growth+c_THR)         * K_dLAC_growth/(K_dLAC_growth+c_LAC) ...
           * K_dNH4_growth/(K_dNH4_growth+c_NH4) * c_ATP/(K_mATP_growth+c_ATP); 
    %1/hr
    %Checked 1/28/18 Ed SEE CORRECTION
%34 BIG AUTHOR TYPO IN THAT TYR IS DOUBLE COUNTED FOR
V_mAb = V_maxmAb * c_LYS/(K_mLYS_mAb+c_LYS) * c_ILE/(K_mILE_mAb+c_ILE) ...
        ...
        * c_THR/(K_mTHR_mAb+c_THR) * c_ATP/(K_mATP_mAb+c_ATP);%EGLN typo 
    %mmol/1E6 cells/hr
    %Checked 1/28/18 Ed
